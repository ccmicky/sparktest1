...
object Arith extends JavaTokenParsers {
  ...
  def expr: Parser[Expr] = term ~ rep("+" ~ term | "-" ~ term| "||" ~ term) ^^ {
  ...
  def factor: Parser[Expr] = stringLiteral ~ "->" ~ stringLiteral ^^{
     x => UnaryOp("->", Variable2(x._1._1,x._2))
  } | ("-"|"!") ~ stringLiteral ^^ { x1 => UnaryOp(x1._1 , Variable(x1._2)) } |stringLiteral ^^ { x => Variable(x) } |  "(" ~> expr <~ ")"
